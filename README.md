...
For a fresh developer setup focused on Singapore, use the local helper script in this repository:
...
- builds the local Docker image from this repo
- downloads the latest `malaysia-singapore-brunei` Geofabrik extract into `mapfiles/`
- creates the `osm-data` and `osm-tiles` Docker volumes
- imports the extract into `osm-data`
- loads the external style data with certificate verification enabled
- applies fallbacks for any remaining low-zoom external tables so the map can still render cleanly if those downloads fail

After startup, open [http://localhost:8080](http://localhost:8080). The demo map is centered on Singapore by default.
...
curl -I http://localhost:8080/tile/11/1614/1009.png
```

Singapore spot checks:

- East Coast Park: [http://localhost:8080/tile/17/103369/65062.png](http://localhost:8080/tile/17/103369/65062.png)
- Bedok Mall: [http://localhost:8080/tile/17/103375/65053.png](http://localhost:8080/tile/17/103375/65053.png)
- Tuas Checkpoint: [http://localhost:8080/tile/17/103270/65045.png](http://localhost:8080/tile/17/103270/65045.png)

If you need to rebuild the volumes from scratch, run:

```
FORCE_REIMPORT=1 ./setup-singapore.sh
```
